...
class gpt_kde_test(cache_test):
    def __init__(self, dataset, embedding_model, must_run=False):
        super().__init__(dataset, embedding_model)
        self.list_store = list_store(rf"{self.dir_path}/data/{self.dataset}_{self.embedding_mod}_gpt_kde_similarities_cache.json")
        self.must_run = must_run

    def test_self(self):
        if os.path.exists(rf"{self.dir_path}/data/{self.dataset}_{self.embedding_mod}_gpt_kde_similarities_cache.json") and not self.must_run:
            print("Loading similarities from cache...")
            load_result = self.list_store.load_list()
            right_similarities = load_result[0]
            wrong_similarities = load_result[1]
        else:
            self.hit_db = vcache_hit_record_SQLiteManager(rf"{self.dir_path}/sqlite_cache.db")
            for i in range(len(self.ds["train"])):
                query_embedding = self.ds["train"][i]["embedding"]
            ...
            self.list_store.save_list([right_similarities, wrong_similarities])
        # 接下来使用核函数展示分布
        print("开始绘制分布图...")
    ...
    def test_self(self):
        if os.path.exists(rf"{self.dir_path}/data/{self.dataset}_{self.embedding_mod}_{self.cache.__class__.__name__}_delta = {self.cache.delta}_better_vcache_get_list_cache.json") and not self.must_run:
            ...
            for i in range(len(self.ds["train"])):
                ...
                if similar_ids:
                    s_vals, c_vals = self.hit_db.search_by_id(similar_ids[0][0])
                    if self.cache.decide(similar_ids[0][1], s_vals, c_vals) == 'exploit' and (self.cache_hit - self.right_hit) / (self.cache_hit + self.miss) < self.delta:
                        self.cache_hit += 1
                        if self.sqllite_db.search_by_id(similar_ids[0][0]) == [self.ds["train"][i][key] for key in self.key_name]:
                            self.right_hit += 1
                    else:
